...
- **`App.tsx`** wraps the `Canvas` component in `ReactFlowProvider`. The `Canvas` component (~760 lines) is the central component handling all logic: paste, drag-and-drop, file upload, node CRUD, selection toolbar, AI modal, and dark mode toggle.
- **No state library** — all state lives in `Canvas` via `useState`/`useRef`. A `nodesRef` mirrors the `nodes` state to avoid stale closures in async callbacks. Viewport (zoom/pan) is persisted to `localStorage`.
