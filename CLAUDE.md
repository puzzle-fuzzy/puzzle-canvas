...
Puzzle Canvas is an infinite canvas app for bookmarking URLs, images, and videos by pasting or drag-and-drop. Built with React 19 + @xyflow/react (Vite 8 with Rolldown) on the frontend and Hono + Bun on the backend, with SQLite via Drizzle ORM.
...
- **`App.tsx`** wraps `Canvas` in `ReactFlowProvider` + `IconProvider`. `Canvas` composes stores, hooks, and registers 4 custom node types (`urlNode`, `imageNode`, `videoNode`, `docNode`).
- **State management** — **zustand** stores in `src/stores/`:
  ...
  - `uiStore` — dark mode, modals (AI, settings, login, fullscreen preview), error toast, theme color (synced to CSS custom properties), toolbar icon size.
...
- **Custom hooks** in `src/hooks/` bridge ReactFlow hooks with zustand stores:
  ...
  - `useNodeActions` — URL node creation (fetches OG metadata), file upload with progress nodes, AI generation.
...
- **Custom node types** in `src/components/` — `UrlNode` (bookmark card with OG metadata), `MediaNode` (image/video, differentiated by `data.type`; video hover-to-play), `DocNode` (file with extension-based icon). Toolbars: `ModeToolbar` (right-side), `SelectionToolbar` (floating, batch ops). Modals: `AIModal`, `SettingsModal` with `IconPanel`.
...
server/
  ...
  db/
    ...
    schema.ts           # 4 tables: users, accounts, refresh_tokens, nodes
...
- `nodes` — polymorphic canvas nodes (urlNode/imageNode/videoNode/docNode). Nullable type-specific columns: `url`/`title`/`description`/`image`/`favicon` for URL nodes; `src`/`fileName`/`fileSize` for media/doc nodes. `userId` FK.
...
- **Environment**: No `.env` files. `JWT_SECRET` defaults to `'dev-secret-change-me'`. Upload dir `./uploads/`. All ports hard-coded (frontend 5175, backend 3001).
